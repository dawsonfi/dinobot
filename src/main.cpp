...
void process_machine_state(char* topic, byte* payload, unsigned int length) {
  ...
  if (payloadStr.indexOf("dinobot=on") >= 0) {
    Serial.println("Turning ON");
    digitalWrite(DINOBOT_ON_SWITCH, HIGH);
  } else if (payloadStr.indexOf("dinobot=off") >= 0) {
    Serial.println("Turning Off");
    digitalWrite(DINOBOT_ON_SWITCH, LOW);
  }
}